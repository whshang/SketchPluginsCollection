...
com.notebook = {
    ...
    addComment: function () {
        ...
        if(elementSelected){
            ...
            textComment.setStringValue("尚未添加描述");
    ...
    // Create sidebar
        ...
        var logo = this.addTxt(header,'Sketch Notebook','#444444',20,'Sketch Notebook',sc.width,24,sc.margin,sc.margin),
        ...
        for (var i = 0; i < mInfo2.length; i++) {
            var label = this.addTxt(m,'label_'+mInfo2[i].label,'#666666',12,mInfo2[i].label.toUpperCase()+":",65,11,0,newY+2,fixed=true),
                value = this.addTxt(m,'value_'+mInfo2[i].value,'#cccccc',14,mInfo2[i].value,360,21,80,newY,fixed=true),
                lid,vid;
            newY = newY+sc.margin;
        ...
        var sLx = midLine.absoluteRect().x(),
            ...
            screenLabel = this.addTxt(sidebar,'label_screen','#666666',11,"界面名称",100,11,0,0);
    ...
    // Create comment
        ...
        var bodyY = title.absoluteRect().y() + title.absoluteRect().height() + 10,
            body = this.addTxt(comment,'comment body','#999999',14,"Comment",400,16,40,bodyY,fixed=true);
        ...
        var iBg = this.addOval(index, 'bg', '#E74C3C', 30, 30, 0, 0);
